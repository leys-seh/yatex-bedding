...
export default function WhatsAppProductButton({
  ...
  const productMessage = dictionary.product.whatsappMessage.replace("{name}", productName);
  ...
  function includeCurrentProductUrl(event: React.MouseEvent<HTMLAnchorElement>) {
    const message = [productMessage, "", dictionary.product.productUrlLabel, window.location.href].join("\n");
  ...
  return (
    <a
      ...
      aria-label={dictionary.product.whatsappAria.replace("{name}", productName)}
    ...
    >
      ...
      <span>{dictionary.product.whatsapp}</span>
      <svg
        aria-hidden="true"
